...
def encrypt_int(key, integer):
    # pick random y from (0, p-1) inclusive
    y = random.randint(0, key.p)
    # c = g^y mod p
    c = modexp(key.g, y, key.p)
    # d = ih^y mod p
    d = (integer * modexp(key.h, y, key.p)) % key.p
    return [c, d]


def decrypt_int(key, ciphertext):
    c = int(ciphertext[0])
...
def reencrypt(key, ciphertext):
    # pick random y from (0, p-1) inclusive
    y = random.randint(0, key.p)
    # c = g^y mod p
    c = (int(ciphertext[0]) * modexp(key.g, y, key.p)) % key.p
    # d = ih^y mod p
    d = (int(ciphertext[1]) * modexp(key.h, y, key.p)) % key.p
    return [c, d]


def divide(cipher1, cipher2, public_key):
    first = cipher1[0] * modexp(cipher2[0], -1, public_key.p) % public_key.p
    second = cipher1[1] * modexp(cipher2[1], -1, public_key.p) % public_key.p
    return [first, second]


def multiply(cipher1, cipher2, public_key):
    first = cipher1[0] * cipher2[0] % public_key.p
    second = cipher1[1] * cipher2[0] % public_key.p
    return [first, second]
